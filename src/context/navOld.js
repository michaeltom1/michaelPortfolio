  ...
  return (
    <div>
      <nav
        ref={menuRef}
        className={`${
          lastScrollTop === 0 ? "" : ` bg-primar backdrop-blur-sm`
        } ${
      ...
      >
        ...
        {/* <div className="backdrop-blur-lg bg-primaryLight lg:hidden absolute w-full top-0 z-0">
          <ul className={``}>
            {navItems.map((item) => (
...
export default Nav;
<div
  className={`backdrop-blur-lg bg-primaryLight md:hidden pt-16 fixed w-full right-0 ease-in duration-300 ${
    menuClick ? "-translate-y-0 top-0" : "-translate-y-full top-0"
  }`}
>
  <ul className={``}>
    {navItems.map((item) => (
